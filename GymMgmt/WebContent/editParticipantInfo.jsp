...
String phNo = (String) session.getAttribute("phonenumber");
...
%>
	<div class="backimage">
		...
		<div class="main">
			...
			<form action="EditParticipantController" method="post">
				<input type="hidden" name="operation"><br /> <label>Participant
					Name</label> &nbsp;&nbsp;&nbsp; <input type="text" name="pname"
					value=<%=name%> required="required"><br /> <label>Age</label>
				&nbsp;&nbsp;&nbsp; <input type="number" name="age" min=15
					value=<%=age %> required="required"><br /> <label>PhoneNumber</label>
				&nbsp;&nbsp;&nbsp; <input type="tel" id="phone" name="phonenumber"
					value=<%=phNo %> required="required"><br />
				<br> <select name="batchid" required="required">
...
while(li.hasNext()){
	Batch bb = li.next();
	%>
					<option value="<%=bb.getBatchid()%>"
						<c:if batchid="<%=id%>"> selected="selected"</c:if>>><%=bb.getBatchid()%>(<%=bb.getTime()%>)
					</option>
					<%
}
%>
